...
## Base R 
As we already mentioned, R comes with capabilities to import, analyze, and export data.  But since R's creation, users have created `packages` which act like plug-ins or addons.  These add or improve the functionality of R.  We'll be using a package suite called `tidyverse` that tries to make R more straightforward for beginners.
...
Remember how we mentioned earlier that data should be "tidy", that is each variable should be represented in one column and each row represents one observation.  A group of packages has been created called the `tidyverse` which can help us work with data in a tidy way.  We saw the first package earlier, the `readr` package.  It helped us read in data and avoid some headaches from base R that you didn't have to deal with.  We are now going to discuss another of these packages, which helps you to manipulate your data, a package called `dplyr`.
