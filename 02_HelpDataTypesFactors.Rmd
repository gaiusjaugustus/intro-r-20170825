...
Cells <- data.frame(Day = c(1, 2, 3, 4, 5), 
           Magnification = factor(c(2, 10, 5, 2, 5), levels = c(2, 5, 10), ordered = TRUE),
           Observation = c("Growth", "Death", "No Change", "Death", "Growth"), stringsAsFactors = FALSE)
```
...
4. **ADVANCED:** Use dplyr or base R to create a new column that multiplies the Magnification by the Day. 

    Answer:
```
Cells <- Cells %>% mutate(NewColumn = Magnification * Day)
```
